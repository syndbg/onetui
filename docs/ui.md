...
This changes presentation only. Byte fields still use their stable `\x...` hexadecimal projection for page-local filter/sort, even when Auto renders readable text. Kafka's explicit [Protobuf/Avro bindings](kafka.md#schema-bound-key-and-value-previews) add separate decoded JSON, schema and error columns; they do not alter the raw key/value fields. Files, directory catalogs and registry IDs resolve on the worker. Reopen the connection to reload local schemas.

Record tables always show compact JSON. Row/detail views honor `pretty_print`: on by default for two-space indentation; off preserves retained whitespace except for escaped terminal controls. Both forms preserve key order, duplicate keys, number text and string escapes. Plain text is not parsed recursively or converted into another format. Data highlighting defaults to on; turning it off retains selection, focus, error colors and UI key hints. Highlight/wrap changes retain the current detail chunk; changing format, indentation or Unicode rendering rebuilds detail from its first chunk.

Word wrapping defaults to on for read-only text throughout the app, including table previews, help and status. Record tables and unfocused inspector fields allow at most three visual lines per preview, with `…` marking truncation; the focused inspector field expands, and Enter opens dedicated detail. A single table column fills the panel. Short columns fit their headings and content; overflowing columns share the remaining width. Widths use the whole loaded page and stay stable during row selection, filtering and sorting. Resizing changes the visible preview, without a fixed character cutoff. With wrapping off, use `H`/`L` for horizontal content scrolling, `h/l` to choose fields and `j/k` to scroll detail/help. Newlines remain logical line boundaries. Command/filter editors stay single-line; structural labels and headings retain their layout constraints. The fixed-height footer may clip long status text.
