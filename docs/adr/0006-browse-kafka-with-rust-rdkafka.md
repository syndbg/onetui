...
Disable automatic commits, automatic offset storage and topic auto-creation. Set `auto.offset.reset=error` and assign explicit offsets rather than consulting committed positions. If the high-level client requires a group ID, generate a private session ID internally; it is not permission to subscribe, commit or reset group offsets. These safety settings are connector-owned, not arbitrary user overrides. See [librdkafka configuration](https://github.com/confluentinc/librdkafka/blob/master/CONFIGURATION.md).

Read committed records by default. Capture a finite upper offset when opening the partition; refresh starts a new browsing window. Offsets are partition-local, not row numbers or a global topic order. Retention, compaction and transactions can leave gaps or prevent reaching the captured end. A timeout or quiet poll is not proof of end-of-data. Report unavailable positions or an incomplete read without silently moving the bookmark. The browsing window is not a retained snapshot.
