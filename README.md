...
| Kafka | [Implemented, read-only](docs/kafka.md) | Broker/topic/partition metadata, broker/topic configuration, consumer groups and members, committed offsets and read-committed lag, record paging and live following, offset/timestamp replay JSON editor, byte-value inspection, headless checks, verified TLS and SASL PLAIN/SCRAM | Cross-partition following, publishing, group administration, schema registry, mutual TLS, OAuth, GSSAPI; hosted release validation pending |
