...
pub fn dump<P: Provider>(catalog: &[P], datasource: Option<&str>) -> Result<String> {
    ...
    Ok(serde_json::to_string_pretty(&serde_json::json!({
        ...
        "diagnostics": "Native PostgreSQL SQLSTATE/message/detail/hint/context, Qdrant gRPC code/message, Kafka errors, NATS server error JSON/codes/descriptions and connection error causes. Known selected-connection secrets are redacted and terminal controls escaped. Transport metadata is excluded. Local validation, cancellation and application limits retain local messages. Always enabled; no TOML setting.",
        ...
        "configuration": {
            "format": "TOML; optional theme string, display table and connections table. Missing connections means an empty picker. Every entry is validated, including unselected aliases; unknown themes/kinds/fields, invalid reference names, relative PostgreSQL CA paths and invalid Qdrant URLs are rejected.",
