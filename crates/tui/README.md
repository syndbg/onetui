...
Enter on a data row opens all fields as a Field / Type / Value list. Select a field and press Enter again for its full cached value. Complete single-value results, including Qdrant payloads, open directly in the value viewer. Esc returns one level; navigation adds no datasource read. These behaviors need no configuration keys.
