...
`e` / `:query` opens a compact editor above retained rows. Ctrl-R or F5 executes; Enter inserts a newline. Results stay visible during editing, loading and errors. Successful execution focuses the table and leaves the executed query visible above it. `e` edits again; Esc closes the editor first, then returns from results to browsing. Screen scrolling uses the remaining table height. See [native query controls and limits](../../docs/queries.md).
