...
Directly below context, a three-row bordered bar holds command/filter input or the applied page filter and lexical sort. Its borders show mode and key hints; long input scrolls horizontally to keep the edited end visible. Below 12 terminal rows, the bar becomes one unbordered line. Input modes do not resize the table or change terminal cursor visibility. Press `/`, type `4b`, then Enter to apply a page filter; Esc discards an edit. Press `:`, type `themes`, then Enter to open the theme menu. The bar also shows theme-preview controls while that menu is open.
