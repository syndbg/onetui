...
The command/filter bar is hidden by default, leaving that space for the table. It appears directly below context during command/filter entry, while a nonempty page filter or local sort is applied, or while the theme menu is open. Clearing the filter and sort hides it again once input closes. Command entry shows only `:` and the typed command inside a plain border. Filter and theme modes retain their relevant hints; general navigation keys stay in context/help. Long input scrolls horizontally to keep the edited end visible. Below 12 terminal rows, the bar becomes one unbordered line. Showing or hiding it resizes the table without changing terminal cursor visibility. Press `/`, type `4b`, then Enter to apply a page filter; Esc discards an edit. Press `:`, type `themes`, then Enter to open the theme menu.
