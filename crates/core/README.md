...
`follow_resources` declares views with live following. `Executor::follow_page` returns a bounded batch with a continuation even when empty; `stop_follow` releases live subscriptions when the user stops, including between batches. The existing deadline/cancellation contract applies. Core does not own polling timers or display buffers. See [ADR-0007](../../docs/adr/0007-follow-live-records-in-bounded-batches.md).
