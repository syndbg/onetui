...
The [Redpanda fixture](../../hack/README.md#redpanda-and-schema-registry) uses the same Kafka provider with a real Schema Registry. Separate Avro and Protobuf tests cover exact writer versions, referenced types, raw preservation, old bookmarks, replay and continued following after unavailable/malformed schemas. `seed_redpanda` produces the local `demo_avro` dataset with the native Apache Avro writer; that writer dependency is test/example-only. Apache Kafka tests and loopback registry fault tests remain independent.
