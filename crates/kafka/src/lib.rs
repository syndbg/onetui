...
fn capabilities() -> serde_json::Value {
    serde_json::json!({
        ...
        "configuration": {
            ...
            "decoders": {
                ...
                "fields": {
                    ...
                    "reader_schema_file": {"required": false, "default": null, "type": "absolute regular-file path, at most 4096 UTF-8 bytes without controls; Avro only", "purpose": "Optional self-contained reader-schema JSON, at most 256 KiB in addition to the writer bundle. Applies to raw, catalog and registry Avro. Omit to show writer values. No expansion or automatic latest schema. Loaded on check/first read and cached until reopening, including failures. Defaults are expansion-bounded before native resolution."},
                ...
                "columns": ["key_decoded", "key_schema", "key_decode_error", "key_native", "key_native_error", "value_decoded", "value_schema", "value_decode_error", "value_native", "value_native_error"],
                ...
                "presentation": "Only bound fields add columns. Native typed inspection preserves Avro union branches/logical types and Protobuf field types, enum numbers and unknown fields with re-encoded bytes. JSON/native errors are independent. Optional Avro readers affect JSON; native inspection shows writer and reader values or reader errors, and schema identity includes the reader hash. Null payloads are not decoded; empty payloads are decoded. Original wire bytes remain authoritative. Each preview is at most 64 KiB; combined page stays within 1 MiB. Budget exhaustion reports errors or a notice without discarding raw fields."
            }
        },
        "safety": "No auto commits, auto offset storage, topic auto-creation or group subscriptions. Explicit numeric assignment; auto.offset.reset=error; read_committed. Internal session group ID is not an application group. No generic native configuration overrides. Publishing is explicit: only a PRODUCE statement writes, it is never retried, and its producer waits for all in-sync replicas.",
        ...
        "config_inspection": "Read-only topic/broker configuration, sorted by name and locally paged; requires DescribeConfigs on the topic or cluster respectively. Includes source, default/read-only/sensitive flags and synonyms in broker precedence order. Synonyms contain name, value and source; an empty list means none returned. Sensitive parent or synonym values and unavailable values are null. No config changes or secret retrieval. Uses the existing native client and request deadline; adds no connection settings.",
        "scope": "Browsing and following use one selected topic, optionally one partition. Cross-topic views are permanently out of scope.",
        "query_statements": {"syntax": "CONSUME topic/partition [offsets|time start..end], or PRODUCE topic[/partition] followed by a blank line and a body. The verb line names the target and, for CONSUME, the whole range; the editor prefills it from the open view.", "consume": {"offsets": "offsets start..end; start defaults to the earliest available offset, end to the current read-committed end", "time": "time start..end; start is nonnegative i64 Unix milliseconds and resolves to an offset, not a per-record time filter", "behavior": "Range clause on the verb line for one named partition; CONSUME takes no body. An omitted range reads the whole retained partition, and either side of start..end may be omitted. CONSUME reads directly by offset: it joins no consumer group and commits no offsets."}, "produce": {"body": "One JSON record per line, at most 100 per submission; blank lines are ignored", "fields": {"key": "Optional string", "value": "Optional string; a record needs a key or a value. An explicit null is a tombstone and requires a key", "partition": "Optional nonnegative i32; overrides the verb line, which overrides the broker partitioner (key hash, else round-robin)", "headers": "Optional object of UTF-8 name/value pairs", "key_encoding": "Optional base64, hex or utf-8 (default) decoding for key", "value_encoding": "Optional base64, hex or utf-8 (default) decoding for value"}, "encoding": "Keys and values are UTF-8 unless an encoding decodes them to bytes; whitespace in base64 and hex payloads is ignored. Schema encoding is not performed: publish already-encoded bytes.", "result": "A write outcome of applied, rejected or unknown, with each delivered record's partition and offset in its summary; no paging", "behavior": "acks=all with retries disabled. Applied requires every record delivered and rejected every one refused by a completed response; a partly delivered batch is unknown, because resending duplicates the records that landed"}},
        ...
        "unsupported": ["global timestamp ordering", "cross-topic browsing (permanently out of scope)", "publishing from the app", "consumer-group administration", "Confluent header-GUID framing", "OAuth opaque tokens, interactive grants and SASL extensions", "Kerberos ticket acquisition, renewal and per-alias caches"]
    })
}
