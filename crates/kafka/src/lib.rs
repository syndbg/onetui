...
fn capabilities() -> serde_json::Value {
    serde_json::json!({
        ...
        "configuration": {
            ...
            "decoders": {
                ...
                "fields": {
                    ...
                    "framing": {"required": true, "values": ["raw", "confluent"], "purpose": "Raw uses a local file; confluent requires Avro and registry, resolves the four-byte schema ID after version byte zero. No automatic detection."},
        ...
        "unsupported": ["global timestamp ordering", "cross-topic browsing", "publishing from the app", "consumer-group administration", "Protobuf schema registry", "Confluent header-GUID framing", "Avro reader-schema resolution", "native decoded-type inspector", "client TLS certificates", "broker OAuth", "GSSAPI"]
    })
}
