...
fn capabilities() -> serde_json::Value {
    serde_json::json!({
        ...
        "values": "Key and value are nullable bytes; empty bytes differ from tombstones. Headers are ordered JSON entries with names and nullable byte arrays, preserving duplicates. Optional raw Protobuf/Avro bindings add decoded JSON, schema identity and per-value error columns; original fields remain unchanged. No schema-registry decoding.",
        ...
        "configuration": {
            ...
            "decoders": {
                ...
                "fields": {
                    ...
                    "framing": {"required": true, "values": ["raw"], "purpose": "No prefix stripping or format detection"},
                    "schema_file": {"required": true, "type": "absolute path, at most 4096 UTF-8 bytes without controls", "purpose": "Regular local writer-schema JSON or binary FileDescriptorSet with imports; at most 256 KiB. No environment expansion, tilde expansion or relative-path resolution."},
                    "message_name": {"required": "protobuf only; forbidden for avro", "type": "exact fully qualified string, 1..1024 UTF-8 bytes without controls"}
                },
                "example": {"topic": "events", "field": "value", "format": "avro", "framing": "raw", "schema_file": "/absolute/event.avsc"},
                "lifecycle": "Config parsing is offline. First relevant read loads the schema on the native worker; --check loads all bindings for the selected alias. Successes and failures are cached until the connection is reopened; refresh does not reload files. No renderer I/O or automatic retries.",
        ...
        "unsupported": ["global timestamp ordering", "cross-topic browsing", "publishing from the app", "consumer-group administration", "schema registry", "Confluent framing", "Avro reader-schema resolution", "native decoded-type inspector", "client TLS certificates", "OAuth", "GSSAPI"]
    })
}
